...
- 内置可配置多实例的 Jav321、JavDB、JavBus、JavLibrary 搜索适配、统一 ResourceRanker 和排序解释。
...
本地临时环境没有稳定的 JavBus 登录态、MetaTube 服务、qBittorrent 下载任务和大体积媒体文件，因此没有在本地实际下载影片。实现使用解析器测试与 mock HTTP 测试验证来源、qBittorrent 和 MetaTube 客户端，并以 NAS 三项服务健康、容器日志和页面 API 做环境验收。

本次运行环境没有可连接的浏览器实例，无法执行截图式视觉回归。前端已完成 Vue 类型检查和 Vite 生产构建，并按 320px、640px、820px、900px、1180px 响应式断点审查；NAS 部署后会再次检查真实页面可达性。

## 已知边界

- JAV 来源依赖外部 HTML 结构，站点改版时需要更新对应适配器；每个来源独立失败并显式降级，不影响本地与其他来源结果。
