...
async fn create_run_with_mode(
    ...
    let (from, to) = if mode == "incremental" && (input.from.trim().is_empty() || input.to.trim().is_empty())
    {
    ...
    let idempotency_key = format!("{mode}:{}:{from}:{to}:v1", input.provider_key);
    ...
    state.task_engine.create_item(run.id, "page:1", first_page).await?;
...
async fn resume_run(
    ...
    Ok(Json(
        state.task_engine.transition(id, JobStatus::Pending, None).await?,
    ))
}
