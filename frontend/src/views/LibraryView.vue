...
const message = useMessage(); const loading = ref(true); const query = ref(''); const items = ref<LibraryItem[]>([]); const page = ref(1); const pageSize = ref(20); const total = ref(0)
async function load() { loading.value = true; try { const result = await api.library(query.value.trim(), page.value, pageSize.value); items.value = result.items; total.value = result.total } catch (reason) { message.error(reason instanceof Error ? reason.message : '媒体库加载失败') } finally { loading.value = false } }
