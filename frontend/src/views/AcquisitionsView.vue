...
function stateLabel(value: string, qbitState?: string | null) { if (value === 'DOWNLOADING' && qbitState) return qbitStateLabel(qbitState); return ({ REQUESTED:'已请求',RESOURCE_RESOLVING:'解析资源',QUEUED:'排队',DOWNLOADING:'下载中',DOWNLOADED:'已下载',PROCESSING:'整理文件',METADATA:'补全元数据',LIBRARY_COMMIT:'提交媒体库',COMPLETED:'已完成',NEEDS_ATTENTION:'需要关注',CANCELLED:'已取消' } as Record<string,string>)[value] ?? value }
