...
const route = useRoute(); const message = useMessage(); const loading = ref(true); const status = ref(''); const page = ref(1); const pageSize = ref(20); const attentionPage = ref(1); const attentionPageSize = ref(20); const acquisitions = ref<Paged<Acquisition>>({ items: [], total: 0, page: 1, pageSize: 20, totalPages: 0 }); const attention = ref<Paged<AttentionItem>>({ items: [], total: 0, page: 1, pageSize: 20, totalPages: 0 }); let events: EventSource | null = null; let refreshTimer: number | undefined
