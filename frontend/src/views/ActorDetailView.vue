...
import { api } from '../api'
import type { Actor, ProductMedia } from '../types'
import PosterCard from '../components/PosterCard.vue'

const route = useRoute(); const router = useRouter(); const message = useMessage(); const loading = ref(true); const changing = ref(false); const actor = ref<Actor | null>(null); const media = ref<ProductMedia[]>([])
const id = computed(() => Number(route.params.id))
async function load() { try { const result = await api.actorDetail(id.value); actor.value = result.actor; media.value = result.media } catch (reason) { message.error(reason instanceof Error ? reason.message : '演员详情加载失败') } finally { loading.value = false } }
